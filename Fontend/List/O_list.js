...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (authUser.role === 'ADMIN') {
        ...
        document.querySelectorAll('a[href*="S_dash.html"]').forEach(a => {
            a.href = '../A_dashboard/A_dash.html';
        });

        // 2. Synchronize navigation bar to match A_dash.html explicitly
        const navLinks = document.querySelector('.nav-links');
        if (navLinks) {
            navLinks.innerHTML = `
                <li><a href="../A_dashboard/A_dash.html" style="text-decoration: none; color: inherit;">Dashboard</a></li>
                <li><a href="../A_confirm/A_confirm.html" style="text-decoration: none; color: inherit;">Confirm</a></li>
                <li class="active">Order List</li>
            `;
        }

        // 3. Remove 'Create New Order' button
        const createBtn = document.querySelector('.btn-primary');
        if (createBtn && createBtn.innerHTML.includes('Create')) {
            createBtn.style.display = 'none';
        }

        // 4. Remove 'Draft' option from the status filter
        const statusFilterEl = document.getElementById('statusFilter');
        if (statusFilterEl) {
            Array.from(statusFilterEl.options).forEach(opt => {
                if (opt.value === 'Draft') opt.remove();
            });
        }
    }
